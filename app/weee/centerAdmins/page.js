...
import {
  postCenterOffice,
  fetchCenterOffices,
} from "../../../app/api/centerOffices/centerOffices";
import { fetchRegions } from "../../../app/api/regions/regions";

export default function CenterAdmins() {
  ...
  async function loadData() {
    try {
      setloading(true);
      const result = await fetchCenterOffices();
      const centerOffices = await result.json();
      setData(centerOffices);
    } catch (err) {
  ...
  const filteredRows =
    data && data.result
      ? data.result.filter(
          (row) =>
            row.centreName.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setloading(true);
      await postCenterOffice({
        regionalOfficesId: regionId,
    ...
    <>
      <section className="flex justify-start items-center flex-col flex-grow w-full rounded">
        <div className="w-full bg-white p-2 md:p-4 rounded-lg">
          <h1 className="mb-2 text-lg font-semibold">Center Office</h1>
          <section className="flex flex-col sm:flex-row justify-center items-center w-full mb-4">
            ...
            <WeeeButton
              loading={false}
              buttonlabel={"Add Center Office"}
              small={true}
              btnaction={async () => await handleOpenModal()}
              makefull={false}
            />
          </section>
          {loading || !data ? (
      ...
      >
        <Modal.Header>Add Center Office</Modal.Header>
        <Modal.Body>
          <form
          ...
          >
            ...
            <div className="w-full mb-2">
              <WeeeInputs
                weeeinputlabel={"Center Name"}
                value={centreName}
                onValueChange={setCentreName}
      ...
      >
        <Modal.Header>Edit Center Office</Modal.Header>
        <Modal.Body>
          <div className="space-y-6">
            <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
              Edit Center Office Fields
            </p>
          </div>
        </Modal.Body>
        <Modal.Footer>
      ...
      >
        <Modal.Header>Delete Center Office</Modal.Header>
        <Modal.Body>
          <div className="space-y-6">
            <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
              Are you sure you want to delete this Center Office?
            </p>
          </div>
        </Modal.Body>
        <Modal.Footer>
